...
impl SplCommand<ast::MvCombineCommand> for MvCombineParser {
    type RootCommand = crate::commands::MvCombineCommand;
    type Options = MvCombineCommandOptions;

    fn parse_body(input: &str) -> IResult<&str, ast::MvCombineCommand> {
        map(
            pair(
                opt(preceded(
