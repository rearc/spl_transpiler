...
impl SplCommand<ast::EventStatsCommand> for EventStatsParser {
    type RootCommand = crate::commands::EventStatsCommand;
    type Options = EventStatsCommandOptions;

    fn parse_body(input: &str) -> IResult<&str, ast::EventStatsCommand> {
        map(
            tuple((
